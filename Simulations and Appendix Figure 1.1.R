# Modello AB-SFC - Multiplicity and not necessarily heterogeneity: implications for the long-run degree of capacity utilization 
...
for(i in 1:Nphi){
  ...
  vvalue=1.25 #1.7 rapporto capitale/lavoro
  datiIn[1,2]=vvalue
  for(j in 1:Nv){
    progv=0
    progval[2]=progv
    ...
    for(h in 1:Nlim){
      ...
      zvalue=45#tempo di vita del capitale
      datiIn[1,5]=zvalue
      for(gg in 1:Nz){
        progz=0
        progval[5]=progz
        zvalue=zvalue+progz   
        dkvalue=6 #8  #numero periodi per la produzione del bene capitale
        datiIn[1,6]=dkvalue
        for(ll in 1:Ndk){
          progdk=0
          progval[6]=progdk
              ...
              for(ww in 1:Nbeta){
                ...
                for(ppp in 1:Nwgov){
                  ...
                  for(popi in 1:Nflex){
                    ...
                    intvalue=0.001 #0.00016  #tasso d'interesse sui titoli 
...
if(NF>2){
yy=(NFC-NFK*cel)/(flo-cel)
xx=NFK-yy
indexcapitale=c(rep(1:xx,each=cel),rep((xx+1):(xx+yy),each=flo))
}
espe=0 #1# se lo metto uguale a zero assumo che la quantita che mi attendo per domani è indipendente dal tasso di crescita atteso.. ma viene calcolato solo utilizzando direttametne le quantità registrate nei periodi precedente (non la variazione dei tassi di crescita)
conta=1
...
cworkercash=0.03 #0.05 #0.04#0.02    #0.06
#propensioneal consumo sui risparmi dei capitlasti 
ccapcash=0.03 #0.05  #0.04 #0.02
...
for (mc in 1:MC) {
  ...
  if(TRUE){
    ...
    meanratio2=matrix(data=0, nrow=T)
    ...
    ratio2=matrix(data=0, nrow=T,ncol=NF)
  ...
  for (t in 1:T){
    ...
    if(TRUE){
      ...
      if(t>dk){    
        ...
        ratio2[t,(NFK+1):NF]=(wb[t,(NFK+1):NF]+sum(kdfeasiblenominal[(t-dk+1):t,(NFK+1):NF])/dk+div[t-1,(NFK+1):NF])/(Gef[t]+Consumptionhh[t])-revenues[t,(NFK+1):NF]/(Gef[t]+Consumptionhh[t])
        meanratio[t]=sum(ratio[t,(NFK+1):NF])/NFC
        meanratio2[t]=sum(ratio2[t,(NFK+1):NF])/NFC
...
#   c(GDPreal,u_raggregate,u_raggregatebigs,u_raggregatesmall,u_raggregatemedium,Cweight,Investmentwight,Gweigth,meanratio,meanratio2,alfasort[NFC],nbancarottac) 
...
#       clusterExport(cl,c("parametri","dati","datiIn","progval","ff","ncomb","eterogeneita","sceltastocastica","sceltastocasticaK","equaldistribution"))
#       res=parSapply(cl,trial, ff)
#       stopCluster(cl)
# #  
# save.image("/Users/lored/Desktop/montecadataespe0un08.RData")     
...
mingdp=min(Investmentvalue[b:T])*0.94
maxgdp=max(Investmentvalue[b:T])*1.06
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(Investmentvalue[b:T], type='l',col=1,lwd=1,main='Investments',lty=1,ylim=range(mingdp,maxgdp),ylab='')
...
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(GDPreal[b:T], type='l',col=1,lwd=1,main='',lty=3,ylim=range(mingdp,maxgdp),ylab='')
par(new=TRUE)
mini=min(interestpaymentG[b:T])*0.985
maxi=max(interestpaymentG[b:T])*0.985
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(interestpaymentG[b:T],xaxt='n',yaxt='n',type='l',ylim=range(mini,maxi),col=c("steelblue"),lwd=1,lty=1,font.main=1,cex.main=0.75,main="",ylab = NA,xlab = NA)
axis(side = 4,col = "steelblue",col.axis="steelblue")
legend("top",ncol=2, c("GDP real","Public debt interests"),  bty = "n", cex = 0.98, lty=c(3,1), lwd=c(1,1), col = c(1,"steelblue"), box.lwd=0)

mingdp=min(EmployeesC[b:T],EmployeesK[b:T])*4*10
maxgdp=max(EmployeesC[b:T],EmployeesK[b:T])*1.02*10
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(EmployeesC[b:T]*10, type='l',col=1,lwd=1,main='Sectoral employment',lty=2,ylim=range(mingdp,maxgdp),ylab='')
par(new=TRUE)
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(EmployeesK[b:T]*40, type='l',col=6,lwd=1,main='Sectoral employment',lty=1,ylim=range(mingdp,maxgdp),ylab='')
legend("right",c("Capital sector","Consumer sector"),  bty = "n", cex = 1.1, lty=c(1,3), lwd=c(1,1), col = c(6,1), box.lwd=0)
...
mingdp=min(debpil[b:T])*0.97
maxgdp=max(debpil[b:T])*1.03
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(debpil[b:T], type='l',col=1,lwd=1,main='Public debt-to-GDP ratio',ylim=range(mingdp,maxgdp),ylab='')
mingdp=min(private_debpil[b:T])*0.97
maxgdp=max(private_debpil[b:T])*1.03
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(private_debpil[b:T], type='l',col=1,lwd=1,main='Private debt-to-GDP ratio',ylim=range(mingdp,maxgdp),ylab='')

mingdp=min(NW_workers[b:T])*0.98
maxgdp=max(NW_workers[b:T])*1.01
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(NW_workers[b:T], type='l',col=1,lwd=1,main='Households wealth',ylim=range(mingdp,maxgdp),ylab='')
par(new=TRUE)
mingdp=min(NW_cap[b:T])*0.98
maxgdp=max(NW_cap[b:T])*1.02
plot(NW_cap[b:T], type='l',yaxt='n',col="steelblue",lwd=1,main='',ylim=range(mingdp,maxgdp),ylab='')
axis(side = 4,col = "steelblue",col.axis="steelblue")
legend("topright",ncol=2,c("Workers wealth","Capitalists wealth"),  bty = "n", cex = 1, lty=c(1,1), lwd=c(1,1), col = c(1,"steelblue"), box.lwd=0)


mingdp=min(BondCB[b:T],Hb2[b:T])*0.98
maxgdp=max(BondCB[b:T],Hb2[b:T])*1.03
#plot(unsatisfieddemand[1:T], type='l',lty=3,lwd=2,col=2,main='',xaxt='n')
#abline(h=0,col="purple",lty=2,lwd=2)
plot(BondCB[b:T], type='l',col=1,lwd=1,main='',ylim=range(mingdp,maxgdp),ylab='')
par(new=TRUE)
plot(Hb2[b:T], type='l',yaxt='n',col="steelblue",lwd=1,main='',ylim=range(mingdp,maxgdp),ylab='')

legend("top",ncol=2,c("Bank reserves","CB public bonds"),  bty = "n", cex = 1, lty=c(1,1), lwd=c(1,1), col = c(1,"steelblue"), box.lwd=0)


mingdp=min(BondDemand_cap[b:T])*0.98
maxgdp=max(BondDemand_cap[b:T])*1.01

plot(BondDemand_cap[b:T], type='l',col=1,lwd=1,main='Houeseholds public bonds',ylim=range(mingdp,maxgdp),ylab='')
mingdp=min(Deposit[b:T])*0.98
maxgdp=max(Deposit[b:T])*1.01
plot(Deposit[b:T], type='l',col=1,lwd=1,main='Deposits',ylim=range(mingdp,maxgdp),ylab='')
